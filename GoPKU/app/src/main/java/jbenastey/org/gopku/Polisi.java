    ...
    private void tampilkanpilihan(String pilihan) {
        try{
            ...
            if (pilihan.equals("Call Center")){
                ...
                polisi = new Intent(Intent.ACTION_DIAL, Uri.parse(tel));
            }
            else if (pilihan.equals("Info Maps")){
//                String geo = "geo:0.482323,101.419719"; //isi dengan posisi polisi
//                polisi = new Intent(Intent.ACTION_VIEW, Uri.parse(geo));
//                //set app default untuk buka intent ini apabila ada beberapa aplikasi yang dapat membuka
//                polisi.setPackage("com.google.android.apps.maps");
                polisi = new Intent(this,PolisiLocation.class);
            }
            else if (pilihan.equals("Driving Direction")){
                String geo = "google.navigation:q=0.518319,101.447838"; //isi dengan posisi polisi
                polisi = new Intent(Intent.ACTION_VIEW, Uri.parse(geo));
            }
            else if (pilihan.equals("Website")){
                String web = "https://www.polri.co.id";
                polisi = new Intent(Intent.ACTION_VIEW, Uri.parse(web));
            }
            else if (pilihan.equals("Search Info")){
                polisi = new Intent(Intent.ACTION_WEB_SEARCH);
                polisi.putExtra(SearchManager.QUERY,("Kantor Polisi Pekanbaru"));
            }
                ...
                polisi = new Intent(Intent.ACTION_VIEW,dekatsaya);
                polisi.setPackage("com.google.android.apps.maps");
            }
            else if (pilihan.equals("Exit")){
                finish();
            }
            startActivity(polisi);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
